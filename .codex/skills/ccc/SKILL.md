...
`ccc` uses a daemon socket and runtime files under `~/.cocoindex_code`, which may be outside an agent's default workspace sandbox.

- **Codex**: Run every `ccc` command with `sandbox_permissions="require_escalated"` from the first attempt. The `ccc` prefix rule is pre-approved, so this should not require a user prompt.
- **Other agents**: Use the agent's native permission mechanism to allow `ccc` to access `~/.cocoindex_code` when sandboxing is active.

Do not run `ccc` inside a known-restricted sandbox first merely to observe the expected permission failure.
